...
namespace MoreLinq
{
    ...
    public static partial class MoreEnumerable
    {
        ...
        /// <param name="source">The sequence to evaluate a sliding screen over</param>
        /// <param name="size">The size (number of elements) in each screen</param>
        /// <returns>A series of sequences representing each sliding screen subsequence</returns>
        ...
        private static IEnumerable<IEnumerable<TSource>> WindowedImpl<TSource>(this IEnumerable<TSource> source, int size)
        {
            ...
            {
                // generate the first screen of items
                var countLeft = size;
                ...
                //       we don't move the iterator once the screen is complete
                while (countLeft-- > 0 && iter.MoveNext())
                {
                    window.Add(iter.Current);
                }

                // return the first screen (whatever size it may be)
                yield return window;

                // generate the next screen by shifting forward by one item
                while (iter.MoveNext())
                {
